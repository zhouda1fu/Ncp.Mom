<template>
  <div class="management-page">
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          <h1>工作中心管理</h1>
          <p class="subtitle">管理系统中的所有工作中心信息</p>
        </div>
        <el-button 
          type="primary" 
          size="large"
          class="create-btn"
          @click="showCreateDialog"
          icon="Plus"
        >
          新建工作中心
        </el-button>
      </div>
    </div>

    <div class="main-content">
      <div class="search-section">
        <div class="section-header">
          <h2>
            <el-icon class="section-icon"><Search /></el-icon>
            搜索筛选
          </h2>
        </div>
        <div class="search-wrapper">
          <el-form :inline="true" :model="searchParams" class="search-form">
            <el-form-item label="搜索">
              <el-input
                v-model="searchParams.keyword"
                placeholder="请输入工作中心编码或名称"
                clearable
                class="search-input"
                :prefix-icon="Search"
                @keyup.enter="handleSearch"
              />
            </el-form-item>
            <el-form-item>
              <el-button type="primary" class="search-btn" @click="handleSearch" icon="Search">
                搜索 
              </el-button>
            </el-form-item>
          </el-form>
        </div>
      </div>

      <div class="table-section">
        <el-table :data="workCenters" v-loading="loading" stripe>
          <el-table-column prop="workCenterCode" label="工作中心编码" min-width="150" />
          <el-table-column prop="workCenterName" label="工作中心名称" min-width="200" />
          <el-table-column label="操作" width="150" fixed="right" align="center">
            <template #default="{ row }">
              <el-button size="small" type="primary" @click="handleEdit(row)" :icon="Edit" />
              <el-button size="small" type="danger" @click="handleDelete(row)" :icon="Delete" />
            </template>
          </el-table-column>
        </el-table>

        <div class="pagination-wrapper">
          <el-pagination
            v-model:current-page="pagination.pageIndex"
            v-model:page-size="pagination.pageSize"
            :total="pagination.total"
            :page-sizes="[10, 20, 50, 100]"
            layout="total, sizes, prev, pager, next, jumper"
            @size-change="handlePageSizeChange"
            @current-change="handlePageCurrentChange"
          />
        </div>
      </div>
    </div>

    <!-- 创建/编辑对话框 -->
    <el-dialog
      v-model="dialogVisible"
      :title="isEdit ? '编辑工作中心' : '新建工作中心'"
      width="500px"
    >
      <el-form :model="formData" :rules="formRules" ref="formRef" label-width="120px">
        <el-form-item label="工作中心编码" prop="workCenterCode">
          <el-input v-model="formData.workCenterCode" placeholder="请输入工作中心编码" />
        </el-form-item>
        <el-form-item label="工作中心名称" prop="workCenterName">
          <el-input v-model="formData.workCenterName" placeholder="请输入工作中心名称" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleSubmit">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { Search, Edit, Delete } from '@element-plus/icons-vue'
import { workCenterApi, type WorkCenter, type CreateWorkCenterRequest, type UpdateWorkCenterRequest } from '@/api/workCenter'

const loading = ref(false)
const workCenters = ref<WorkCenter[]>([])
const dialogVisible = ref(false)
const isEdit = ref(false)
const formRef = ref()
const currentWorkCenter = ref<WorkCenter | null>(null)

const searchParams = ref({
  keyword: ''
})

const pagination = ref({
  pageIndex: 1,
  pageSize: 10,
  total: 0
})

const formData = ref({
  workCenterCode: '',
  workCenterName: ''
})

const formRules = {
  workCenterCode: [{ required: true, message: '请输入工作中心编码', trigger: 'blur' }],
  workCenterName: [{ required: true, message: '请输入工作中心名称', trigger: 'blur' }]
}

const loadWorkCenters = async () => {
  loading.value = true
  try {
    const res = await workCenterApi.getWorkCenters({
      ...searchParams.value,
      pageIndex: pagination.value.pageIndex,
      pageSize: pagination.value.pageSize
    })
    workCenters.value = res.data.items || []
    pagination.value.total = res.data.total || 0
  } catch (error) {
    ElMessage.error('加载工作中心列表失败')
  } finally {
    loading.value = false
  }
}

const handleSearch = () => {
  pagination.value.pageIndex = 1
  loadWorkCenters()
}

const handlePageSizeChange = () => {
  loadWorkCenters()
}

const handlePageCurrentChange = () => {
  loadWorkCenters()
}

const showCreateDialog = () => {
  isEdit.value = false
  formData.value = { workCenterCode: '', workCenterName: '' }
  dialogVisible.value = true
}

const handleEdit = (row: WorkCenter) => {
  isEdit.value = true
  currentWorkCenter.value = row
  formData.value = {
    workCenterCode: row.workCenterCode,
    workCenterName: row.workCenterName
  }
  dialogVisible.value = true
}

const handleDelete = async (row: WorkCenter) => {
  try {
    await ElMessageBox.confirm('确定要删除该工作中心吗？', '提示', {
      type: 'warning'
    })
    await workCenterApi.deleteWorkCenter(row.id)
    ElMessage.success('删除成功')
    loadWorkCenters()
  } catch (error: any) {
    if (error !== 'cancel') {
      ElMessage.error('删除失败')
    }
  }
}

const handleSubmit = async () => {
  await formRef.value.validate()
  try {
    if (isEdit.value && currentWorkCenter.value) {
      await workCenterApi.updateWorkCenter({
        workCenterId: currentWorkCenter.value.id,
        ...formData.value
      })
      ElMessage.success('更新成功')
    } else {
      await workCenterApi.createWorkCenter(formData.value)
      ElMessage.success('创建成功')
    }
    dialogVisible.value = false
    loadWorkCenters()
  } catch (error) {
    ElMessage.error(isEdit.value ? '更新失败' : '创建失败')
  }
}

onMounted(() => {
  loadWorkCenters()
})
</script>

<style scoped>
.management-page {
  padding: 20px;
}

.page-header {
  margin-bottom: 20px;
}

.header-content {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.title-section h1 {
  margin: 0;
  font-size: 24px;
}

.subtitle {
  margin: 5px 0 0 0;
  color: #666;
}

.search-section {
  background: #fff;
  padding: 20px;
  border-radius: 4px;
  margin-bottom: 20px;
}

.table-section {
  background: #fff;
  padding: 20px;
  border-radius: 4px;
}

.pagination-wrapper {
  margin-top: 20px;
  text-align: right;
}
</style>

