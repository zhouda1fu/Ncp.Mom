...
public record StartProductionPlanRequest(ProductionPlanId Id);
...
public class StartProductionPlanEndpoint(IMediator mediator) : Endpoint<StartProductionPlanRequest, ResponseData<bool>>
{
    /// <summary>
    /// 配置端点的基本设置
    /// 包括HTTP方法、认证方案、权限要求等
    ...
    {
        // 设置HTTP POST方法，用于启动生产计划
        Post("/api/production-plans/{id}/start");
    ...
    public override async Task HandleAsync(StartProductionPlanRequest req, CancellationToken ct)
    {
        // 创建启动生产计划命令对象
