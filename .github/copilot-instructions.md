...
Ncp.Mom.sln
├── src/
│   ├── Ncp.Mom.Domain/         # 领域层 - 聚合根、实体、领域事件
│   ├── Ncp.Mom.Infrastructure/ # 基础设施层 - EF配置、仓储接口、仓储实现
│   └── Ncp.Mom.Web/           # 表现层 - API、应用服务
└── test/                            # 测试项目
    ├── Ncp.Mom.Domain.UnitTests/         # 领域层测试项目
    ├── Ncp.Mom.Infrastructure.UnitTests/ # 基础设施层测试项目
    └── Ncp.Mom.Web.UnitTests/           # 表现层测试项目
```
...
### 文件组织
- **聚合根** → `src/Ncp.Mom.Domain/AggregatesModel/{AggregateFolder}/`
- **领域事件** → `src/Ncp.Mom.Domain/DomainEvents/`
- **仓储** → `src/Ncp.Mom.Infrastructure/Repositories/`
- **实体配置** → `src/Ncp.Mom.Infrastructure/EntityConfigurations/`
- **命令与命令处理器** → `src/Ncp.Mom.Web/Application/Commands/`
- **查询与查询处理器** → `src/Ncp.Mom.Web/Application/Queries/`
- **API端点** → `src/Ncp.Mom.Web/Endpoints/`
- **领域事件处理器** → `src/Ncp.Mom.Web/Application/DomainEventHandlers/`
- **集成事件** → `src/Ncp.Mom.Web/Application/IntegrationEvents/`
- **集成事件转换器** → `src/Ncp.Mom.Web/Application/IntegrationEventConverters/`
- **集成事件处理器** → `src/Ncp.Mom.Web/Application/IntegrationEventHandlers/`
...
**Web层** (`src/Ncp.Mom.Web/GlobalUsings.cs`):
...
**Infrastructure层** (`src/Ncp.Mom.Infrastructure/GlobalUsings.cs`):
...
**Domain层** (`src/Ncp.Mom.Domain/GlobalUsings.cs`):
...
**查询处理器**:
```csharp
using Ncp.Mom.Domain.AggregatesModel.{AggregateFolder};
...
```csharp
using Ncp.Mom.Domain.AggregatesModel.{AggregateFolder};
using Ncp.Mom.Web.Application.Commands.{FeatureFolder};
using Ncp.Mom.Web.Application.Queries.{FeatureFolder};
```
